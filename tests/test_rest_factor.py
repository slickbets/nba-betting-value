...
class TestRestAdjustment:
    ...
    def test_back_to_back_is_negative(self):
        ...
        assert adj < 0
        assert adj == -35  # ~1.9 point penalty
...
class TestRestImpactOnSpread:
    ...
    def test_b2b_vs_rested_difference(self):
        """B2B vs well-rested should be ~2.4 point swing."""
        b2b_adj = get_rest_adjustment(0)  # -35
        rested_adj = get_rest_adjustment(3)  # +8

        # Difference of 43 Elo points = ~2.4 point spread swing
        diff = rested_adj - b2b_adj
        assert diff == 43  # Significant but not overwhelming
